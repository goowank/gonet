	...
	var methods = {
		...
		init : function( options )
		{
		    ...
		    $(document.body).mousedown(function(event) {
		        ...
		        if (!target.parents().addBack().is('#specialchar') && (target.closest('a').html()!='<i class="material_icons">star</i>')) { //Clicked outside
		        ...
		        if (!target.parents().addBack().is('#paletteCntr') && (target.closest('a').html()!='<i class="material-icons">format_text_color</i>')) { //Clicked outside
		...
		createTableContext: function(event,cMenuUl){
			...
			cMenuUl.append($('<li/>',{class:"dropdown-submenu",css:{display:"block"}})
       						...
       						.append($('<ul/>',{class:"dropdown-menu"})
       								.append($('<li/>').addClass('dropdown-item').append($('<a/>',{
											id:"tbl_addrow",
											"href":"javascript:void(0)",
											"text":"Add Row"
											}).click(function(e){
		...
		createMenuItem: function(itemSettings, options, returnElement){
			...
			if(itemSettings["select"]){
				...
				$.each(options,function(i,v){
					var option = $('<li class="dropdown-link"/>')
		            $("<a class='nav-link' />",{
		              tabindex : "-1",
		              href : "javascript:void(0)"
		    	...
		    	if(itemSettings["icon"])
					menuElement.addClass("material-icons").html(itemSettings["icon"]);
				else
				...
				var menuWrapElement = $("<a/>",{href:'javascript:void(0)', class:'btn btn-sm btn-outline-secondary'});
				var menuElement = $("<i/>");
				if(itemSettings["icon"])					
					menuElement.addClass("material-icons").html(itemSettings["icon"]);
				else
